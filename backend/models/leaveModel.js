...
const getUsersOnLeaveToday = async () => {
  return await leaveRequestRepository.getUsersOnLeaveToday();
};

// Get team leave data
const getTeamLeave = async (userIdArray, month, year,role) => {
  return await leaveRequestRepository.getTeamLeave(userIdArray, month, year,role);
};

// Get leave balance by user and year
const getLeaveBalance = async (userId, year) => {
  return await leaveBalanceRepository.getLeaveBalanceByUserAndYear(userId, year);
};
...
const requestLeave = async (userId, leaveTypeId, startDate, endDate, isHalfDay, halfDayType, reason) => {
  ...
  const balances = await leaveBalanceRepository.getLeaveBalanceByUserAndYear(userId, new Date(startDate).getFullYear());
  ...
  let initialStatus = leaveTypeId === 9 ? "Approved" : finalApprovalLevel > 1 ? LeaveStatus.PENDING_L1 : LeaveStatus.PENDING;
...
const getLeaveHistory = async (userId) => {
  return await leaveRequestRepository.getLeaveHistoryByUserId(userId);
};
...
const getIncomingRequests = async (userId) => {
  return await leaveRequestRepository.getIncomingRequests(userId);
};
...
const rejectLeave = async (requestId) => {
  return await leaveRequestRepository.updateLeaveRequestStatus(requestId, LeaveStatus.REJECTED);
};

// Create a new leave type
const addLeaveType = async (name, maxPerYear, multiApprover) => {
  return await leaveTypeRepository.createLeaveType(name, maxPerYear, multiApprover);
};

// Update an existing leave type
const updateLeaveType = async (id, name, maxPerYear, multiApprover) => {
  return await leaveTypeRepository.updateLeaveType(id, name, maxPerYear, multiApprover);
};
